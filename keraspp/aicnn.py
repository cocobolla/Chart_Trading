...
class DataSet:
    def __init__(self, X, y, nb_classes, scaling=True,
        ...
        """
        X is originally vector. Hence, it will be transforemd
...
class DataSetModified:
    def __init__(self, X, y, nb_classes, scaling=True,
        ...
        if scaling:
            ...
            scaler = MinMaxScaler()
            n = X.shape[0]
            X = scaler.fit_transform(X.reshape(n, -1)).reshape(X.shape)
            self.scaler = scaler
...
class Machine():
    ...
    def fit(self, epochs=100, batch_size=128, verbose=1):
        data = self.data
        model = self.model
